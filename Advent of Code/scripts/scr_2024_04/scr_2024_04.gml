...
function _2024_04p2(_lines) {
	var _luta = [ord("M"), ord("A"), ord("S")], _luta0 = _luta[0];
	var _lutb = [ord("S"), ord("A"), ord("M")], _lutb0 = _lutb[0];
	...
	for (var _j = 0; _j < _h; _j++) {
		for (var _i = 1; _i < _w; _i++) {
			...
			if ((_bytea != _luta0) and (_bytea != _lutb0)) continue;
			// bottom-left
			var _byteb = string_byte_at(_lines[_j + 2], _i);
			if ((_byteb != _luta0) and (_byteb != _lutb0)) continue;
			// criss-cross crawl
			var _lutai = ((_bytea == _luta0) ? _luta : _lutb);
			var _lutbi = ((_byteb == _luta0) ? _luta : _lutb);
